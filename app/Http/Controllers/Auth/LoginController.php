    ...
    public function customer_login_action(Request $request){
        ...
        if (Auth::attempt($credentials)) {
            ...
            if (Auth::user()->status == 1) {
                        ...
                        Auth::logout();
                                // Toastr::error('Your account is Deactivated by Admin!');
                                Toastr::error('Your account is Deactivated by Admin!', 'Error', ["positionClass" => "toast-bottom-right"]);
                                return redirect()->back();
                    }
        }else{
            Auth::logout();
                    // Toastr::error('Invalid Credentials');
                    Toastr::error('Invalid Login Credentials', 'Error', ["positionClass" => "toast-bottom-right"]);
                    return redirect()->back();
        }
    }
